...
Pure Rust port of the on-chain LunarBase v1 quoting math with v2-style linear
size slippage (**Q64.96** sqrt-price design). Bit-for-bit identical with the
Solidity reference, validated by deterministic and fuzz vectors generated by
...
| `quote_slippage_bps(...)` / `apply_slippage(...)`                         | Direct mirrors of the linear slippage helpers.       |
...
```sh
cargo test --release   # unit + contract-generated deterministic/fuzz vectors
```
...
`concentration_k` keeps the contract's legacy Q20.12 encoding. Its effective
linear coefficient is `concentration_k / 4096`; use
`plain_to_q12_concentration_k` for operator-facing values. Slippage is linear
in swap cash value, uses nested ceiling division, and is capped at 10%.
Directional Q24 fees are intentionally independent and are applied after
slippage, matching v1 `SwapLib`.
